...
The foundation and styled consumer fixtures must continue to exclude `@codemirror/*`, `@lezer/highlight`, and `pdfjs-dist`. This is asserted for every checked-in fixture in `tools/consumer-fixtures/`: the runner derives each fixture's forbidden set from its declared peer group, so any peer-group marker outside that group must be absent from both `node_modules` and the pnpm store.
