...
- `packages/angular/core/resize-behavior.ts` owns a small, framework-agnostic adjacent-pair resize model: min-size clamping, total-preserving deltas, horizontal/vertical coordinates, RTL horizontal inversion, arrow/Home/End key intents, pointer capture/listeners, commit callbacks, and dynamic pair lookup.
- `packages/angular/resizable/resizable.ts` uses that shared runtime for flex split panes. A handle lives between sibling panes, exposes `role="separator"`, `aria-orientation`, `aria-valuemin/max/now`, `aria-controls`, pointer drag, and keyboard resize.
