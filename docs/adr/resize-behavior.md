...
HELL-036 marked resize behavior as a manual runtime risk and asked HELL-046 to compare the current code with Angular CDK DragDrop before production readiness work blesses it. HELL-044 historically constrained table utilities to semantic table helpers; HELL-122 now supersedes the later composable-table direction with `/table` primitives plus `/table-tanstack`. The resize decision must keep separator-style resize narrow and must not reopen grid keyboard, column-model, or data-source ownership outside the primitive/table-shell boundary.
...
- The legacy `projects/hell/src/lib/features/table-utilities/table-utilities.ts` path uses the same runtime for adjacent `th[hellTableHeaderCell]` pairs; HELL-067 replaces that table-specific API with the modern `hellTableResizeHandle` primitive.
...
Do **not** replace it with CDK DragDrop in this slice. CDK DragDrop should remain the first choice for future draggable/reorderable/drop-list interactions, but it is not a drop-in replacement for the current resize contract because Hell must update two caller-owned layout items, preserve their combined size, expose splitter/separator keyboard semantics, and support table-specific adjacent header width transactions.
...
- HELL-046 closes the comparison question with a keep+test decision.
- HELL-061 is blocked as the legacy table-resizer-naming follow-up.
- HELL-084 is the active follow-up for split-pane and modern table resize browser contracts.
...
- If HELL-084 finds browser behavior that unit tests miss and CDK DragDrop can cover with less code, reopen this ADR and create a replace/shrink slice instead of patching broadly.

## HELL-122 table shell note

HELL-122 supersedes the legacy table-utilities and later composable-table boundaries. The resize-behavior decision still applies to the shared separator-style runtime, but table resize remains a `/table` primitive affordance or TanStack-shell styling seam, not a Hell column model. Use HELL-125 for the primitive boundary and HELL-129 for the final docs/gate proof.
