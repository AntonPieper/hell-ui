...
- Date: 2026-05-29
- Slice: HELL-037
- Updated for: HELL-057 on 2026-06-03; HELL-058 on 2026-06-03

## Context

HELL-036 marked `floating-dismissal` as a delegate-first runtime: Hell should shrink document-level outside-click, outside-focus, Escape, and focus-restore code whenever Angular CDK, Angular Aria, or ng-primitives can own the behavior.

The narrow spike target was `HellFlyout` because it was the smallest remaining public primitive that directly creates `HellFloatingInteractionController`. `HellDialog` and `HellPopover` already delegate close-on-outside-click and close-on-Escape to ng-primitives. `HellOmnibar` is a larger composite and already delegates outside overlay clicks to CDK; after HELL-058 it uses the lower-level focus-only Hell dismissal controller instead of the full `HellFloatingInteractionController` lifecycle.
...
Do **not** migrate `HellFlyout` to CDK Overlay, Angular Aria, or ng-primitives in this slice.

The direct-delegation experiment found useful owners for parts of the behavior, but no narrow drop-in owner for the current `HellFlyout` contract:
...
| ng-primitives popover/dialog | Overlay registry, dismiss guards, focus/overlay lifecycle for template-driven primitives. | `NgpPopoverTrigger` owns a `TemplateRef` overlay and dismisses against trigger/overlay/anchor. Adopting it would require converting the inline `hellFlyout` panel into an ng-primitives popover template and reconciling Hell's boundary/scope semantics. That is a follow-up migration decision, not a reversible one-file spike. |

## HELL-037 interim decision

HELL-037 kept `HellFlyout` on the current Hell dismissal adapter as an interim manual path until browser dismissal contracts existed. HELL-057 below is the current post-contract flyout decision.

This was not a decision to keep all floating dismissal forever. It was a decision that HELL-037 should not perform a broad rewrite without browser-level evidence for outside click, outside focus, nested floating surfaces, and focus restoration races.

The spike is backed by focused unit coverage in `projects/hell/src/lib/primitives/flyout/flyout.spec.ts`:
...
Those tests name the behavior a future delegate must preserve before `HellFlyout` can migrate.

## HELL-057 decision: keep the flyout manual adapter

HELL-057 chooses the **keep the manual adapter** path for `HellFlyout`. No CDK/ng-primitives migration slices are created because migration is not chosen, and no broad floating rewrite is authorized.

Evidence considered after the HELL-040 browser contract:

- HELL-040 exercises real docs-harness flyout and omnibar surfaces for pointerdown-inside/focusout/click ordering, delayed guard expiry, outside pointer dismissal, outside focus dismissal, Escape, nested flyouts, and portaled floating scopes. It asserts no double-close, no unsafe focus restore, and records event order on failure; closure notes record passing Chromium/Firefox/WebKit coverage.
...
- HELL-040 makes that contract observable at browser level. The tested behavior is small enough to keep as a named manual adapter, and the available delegate APIs would require public DOM/API migration rather than a bounded replacement.
...
- Any future flyout migration must preserve the HELL-040 browser contract and the `boundary` / scoped-inside semantics in one bounded slice before product code changes.

## HELL-058 decision: keep omnibar focus dismissal composite-owned, but reduce the controller seam

HELL-058 keeps the remaining omnibar outside-focus dismissal as a composite-owned contract. It does not migrate the omnibar to CDK, Angular Aria, or ng-primitives because this slice is only about focus dismissal, not a command-palette DOM/API rewrite.
...
- HELL-040 browser coverage already exercises the real docs harness for portaled omnibar panel focus staying inside and true outside focus closing.
...
- The implementation removes `HellOmnibar`'s direct `HellFloatingInteractionController` usage. Omnibar now constructs `HellFloatingDismissController` directly with `hellOutsideFocus`, the host root, the omnibar floating scope, and an owner document from the host element. The full interaction lifecycle remains a named flyout exception after HELL-057.
...
- HELL-040 — done; browser outside-click and focus-race contracts now guard floating layers.
- HELL-057 — done; `HellFlyout` keeps the manual adapter as a narrow, tested exception.
- HELL-058 — done; omnibar keeps a focus-only composite-owned dismissal seam, removes direct `HellFloatingInteractionController` usage, and tests nested registered floating-surface focus.
- HELL-038 and HELL-039 continue to verify already-delegated dialog/popover and keyboard/focus patterns from the a11y side.
...
- If a future ng-primitives or Angular Aria release exposes an inline dismissible-layer primitive with custom inside-boundary support, reopen this ADR and replace the manual flyout path in a bounded slice.
