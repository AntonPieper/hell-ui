...
| 2 | `chunk-YT2UJ5RK.js` | 378.37 kB | Superseded table docs chunk | Historical pre-HELL-122 evidence for the removed table docs route. Current table docs live at `/components/table` and use `/table` primitives plus the TanStack shell. |
...
| 6 | `chunk-WED6VJIJ.js` | 51.87 kB | Table docs page (`/components/table`)<br>Accessibility guide page | `dist/hell/fesm2022/hell-ui-angular-table.mjs` (51.10 kB) |
...
Lazy owner summary: the largest lazy chunks are correctly behind feature/page boundaries. The expensive lazy owners are Code editor (CodeMirror/Lezer), PDF viewer (pdf.js core/viewer/runtime), table docs examples, and Audio player (Hell audio runtime).
...
| Table docs lazy chunk carries demo/raw source cost | `table-page` includes live examples plus `?raw` source text, table primitives, and the TanStack shell examples. | Table docs | HELL-122 replaces the old table route with `/table` primitives plus `/table-tanstack`; HELL-050 verifies docs examples stay behind lazy routes. |
