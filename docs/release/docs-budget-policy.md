...
| --- | ---: | --- | --- | --- |
| Initial bundle warning | 870 kB | Docs shell / global styles | HELL-030 showed the overage is the docs shell baseline; HELL-031 removed the PDF component-style warning; HELL-043 adds one catalog/search route for the accessibility matrix; HELL-047 hardens the existing docs-shell search hotkey path without adding a new eager dependency; HELL-050 adds a static lazy-route import graph guard for docs examples; HELL-087 keeps shared docs code previews lazy; HELL-122/HELL-129 replace the old table docs route with `/components/table` examples for `/table` primitives and the TanStack shell. HELL-120 deliberately loads `@hell-ui/angular/styles/table` globally because production Vercel evidence showed the previous route-level TypeScript CSS import produced unreferenced CSS and unstyled tables. The pagination/split-view navigation slice adds shared component CSS for compact page status/jump controls and item navigation without adding eager docs route imports. The slider/toggle/avatar polish slice adds global component state CSS for vertical slider containment, dark selected toggle contrast, and avatar overflow hover/open/focus states without new eager docs imports or dependencies. HELL-131 adds a packaged Button Part Style Map recipe utility layer to primitive CSS, so consumers that import Hell primitive CSS receive Button defaults without scanning package source; the docs shell also pays the shared `tailwind-merge` runtime because Button is imported eagerly by shell/navigation examples. The slice keeps the legacy `.hell-button` helper only for non-migrated Toggle/Pagination controls that still use it. This is accepted only as an alpha docs-shell overage, not as permission for unrelated eager imports. | HELL-050 static guard blocks new eager docs example imports; HELL-120 architecture guard blocks reverting to route TS table CSS imports; HELL-133 owns global Part Style Map gates and compiled-CSS packaging proof. Expires immediately if the accepted ceiling, initial error budget, or production-readiness budget decision is reached. |
