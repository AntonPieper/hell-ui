...
```bash
HELL_PACKAGE_CONSUMER_SCENARIOS=root-core,core,testing,button-ui,button,primitive-icons-css,composite-css,app-shell,resizable,split-view,audio-player,audio-transcript,table,table-tanstack,table-tanstack-virtual,no-legacy-alias,code-editor,pdf-viewer pnpm run test:package-consumer -- --minimal-deps
