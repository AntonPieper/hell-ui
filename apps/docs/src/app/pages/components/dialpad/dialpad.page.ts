  ...
  template: `
    <article class="hd-prose">
      ...
      <ul>
        ...
        <li>Two cases deliberately fall back to appending. Tabbing into the display selects the whole number the way any text field does, but that is the browser announcing focus rather than a range you chose, so it is ignored — the next key press or typed digit adds a digit at the caret the display already had instead of replacing everything. Selecting a range yourself, by dragging or with <code>Ctrl</code>/<code>Cmd</code>+<code>A</code>, still replaces it. Writing a new <code>[value]</code> from outside also drops the tracked caret, since the number the caret pointed into is gone.</li>
