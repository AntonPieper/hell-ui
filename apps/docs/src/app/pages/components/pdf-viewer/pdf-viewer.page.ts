  ...
  template: `
    <article class="hd-prose">
      ...
      <ul>
        ...
        <li>In the thumbnail rail, each thumbnail is a button labeled "Go to page N"; the current page carries <code>aria-current="page"</code> and the rail scrolls to keep it in view.</li>
        <li>The rail mounts only the pages around the visible window, so a long document costs a handful of buttons rather than one per page. The mounted buttons form a <code>role="list"</code> inside the rail's landmark, and each carries <code>aria-posinset</code>/<code>aria-setsize</code> so the true page count survives virtualization. The current page and any focused thumbnail stay mounted wherever the window sits, so scrolling the rail never drops focus.</li>
