...
**Scoped Modality**
The blocking contract of a scoped dialog: the Dialog Scope root is the whole blocked region — inert and scroll-locked — while everything outside it keeps focus, pointer input, and its place in the accessibility tree, the panel reports `aria-modal="false"` so assistive technology is not told to ignore that region, and every surface the surrounding shell can open layers above the dialog. It is reference counted per scope root, so simultaneous scoped dialogs block once and the last close restores the exact prior state. It never applies while a page-blocking dialog is open in the same document: that dialog means to block the shell and keeps full-page focus containment and `aria-modal="true"`. It is a property of the open set, not of one dialog's lifetime, so it is re-derived whenever a dialog opens or closes — whichever order they happen in, a document whose open dialogs are all scoped leaves the shell reachable.
_Avoid_: Page-wide `aria-hidden` for a scoped dialog, `aria-modal="true"` on a scoped panel, focus guard instead of `inert`, document-wide focus-trap release while any page-blocking dialog is open, per-open state that only one open/close order restores.
