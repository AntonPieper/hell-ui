  ...
  template: `
    <div class="grid gap-3">
      ...
      <div #scrollHost class="relative h-[420px] overflow-auto rounded-md border border-hell-border bg-hell-surface-subtle">
        <div class="relative min-w-max" [style.height.px]="virtualRows.totalSize()">
          @for (item of virtualRows.virtualItems(); track item.key) {
            @if (item.part; as part) {
              <div
                class="absolute w-full px-2 py-1"
                [style.transform]="'translateY(' + item.start + 'px)'"
                [hellTableMeasureRow]="part"
                [hellTableMeasureRowCallback]="measureVirtualRow"
              >
                @if (part.kind === 'row') {
                  <div class="flex flex-wrap items-center justify-between gap-3 rounded-md border border-hell-border bg-hell-surface-subtle px-3 py-2 text-sm">
                    <div>
                      <strong>{{ part.row.original.name }}</strong>
                  ...
                  <div class="rounded-md border border-hell-border bg-hell-primary-soft p-3 text-sm text-hell-primary">
