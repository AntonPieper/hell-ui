...
// `node tools/release/release-projection.mjs` on a bare checkout without installing
// node_modules, so this module must not import tools that pull in the yaml
// parser (tools/release/release-changelog.mjs → tools/release/change-fragments.mjs → yaml).
...
// Mirrors packageManifestPath in tools/release/release-changelog.mjs (not imported;
...
function main(argv) {
  ...
  switch (command) {
    ...
    default:
      console.error('Usage: node tools/release/release-projection.mjs <barrier|plan|decide|verify|policy> ...');
      return 2;
  }
}
...
function runDecide(args) {
  ...
  if (!expectedPath || !existingPath) {
    console.error('Usage: node tools/release/release-projection.mjs decide <expected.json> <existing-releases.json>');
    return 2;
  }
...
function runVerify(args) {
  ...
  if (!expectedPath || !releasePath || !phase) {
    console.error(
      'Usage: node tools/release/release-projection.mjs verify <expected.json> <release.json> ' +
...
function runPolicy(args) {
  ...
  if (!policyPath) {
    console.error('Usage: node tools/release/release-projection.mjs policy <immutable-releases.json>');
    return 2;
  }
