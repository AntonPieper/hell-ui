  ...
  for (const [symbol] of allowlisted) {
    ...
    failures.push(
      `${specifier}: stale ae-forgotten-export allowlist entry "${symbol}" — the report no longer warns ` +
        'for it; delete the entry from tools/api-report/check-api-report-warnings.mjs',
    );
  }
  ...
  for (const match of reportText.matchAll(namespaceImportPattern)) {
    ...
    failures.push(
      `${specifier}: namespace import of '${match[1]}' — internal cross-entrypoint contracts must be ` +
        'named imports classified in tools/api-report/check-api-report-warnings.mjs',
    );
  }
    ...
    for (const symbol of symbols) {
      ...
      failures.push(
        `${specifier}: unclassified internal contract "${symbol}" from '${from}' — hide the reference ` +
          'or classify the shared contract with a rationale in tools/api-report/check-api-report-warnings.mjs',
      );
    }
  }
    ...
    for (const symbol of entry.symbols) {
      ...
      failures.push(
        `${specifier}: stale internal contract classification "${symbol}" from '${entry.from}' — the ` +
          'report no longer imports it; delete it from tools/api-report/check-api-report-warnings.mjs',
      );
    }
  }
...
export function validateWarningGateConfiguration(reportSpecifiers) {
  ...
  for (const specifier of forgottenExportAllowlist.keys()) {
    ...
    failures.push(
      `forgotten-export allowlist names unknown report specifier '${specifier}' in tools/api-report/check-api-report-warnings.mjs`,
    );
  }
  ...
  for (const entry of classifiedInternalContracts) {
    if (!specifiers.has(entry.report)) {
      failures.push(
        `internal contract classification names unknown report specifier '${entry.report}' in tools/api-report/check-api-report-warnings.mjs`,
      );
    }
