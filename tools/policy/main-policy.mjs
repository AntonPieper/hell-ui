...
// wrap this file — tools/policy/check-main-policy.mjs and
// tools/policy/restore-main-policy.mjs.

import { noConsumerChangeLabel, releasePreparationLabel } from '../merge-state/pr-state-policy.mjs';
    ...
    if (!known.includes(key)) {
      errors.push(
        `${policyRelativePath} records the unknown top-level field ${key}, which nothing compares ` +
          `or restores; remove it, or teach tools/policy/main-policy.mjs to enforce it.`,
      );
    }
  }
}
    ...
    if (!recordedProjectSettings.includes(key)) {
      errors.push(
        `${policyRelativePath} records the unknown project setting ${key}; add it to ` +
          'recordedProjectSettings in tools/policy/main-policy.mjs, or drop it.',
      );
      continue;
    }
      ...
      if (!known.includes(key)) {
        errors.push(
          `${field} rule "${rule.name}" records ${key}, which nothing compares or restores; ` +
            `remove it, or teach tools/policy/main-policy.mjs to enforce it.`,
        );
      }
    }
  }
      ...
      if (!['name', 'color', 'description'].includes(key)) {
        errors.push(
          `Label "${label.name}" records ${key}, which nothing compares or restores; remove it, ` +
            'or teach tools/policy/main-policy.mjs to enforce it.',
        );
      }
    }
  }
